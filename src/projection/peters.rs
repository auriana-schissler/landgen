use crate::geometry::Vertex;
use crate::render::color::render_pixel;
use crate::render::{commit_render_data, RenderState, ThreadState};
use chrono::prelude::*;
use std::f64::consts::PI;
use std::rc::Rc;
use std::sync::Arc;

pub fn render(thread_id: usize, render_state: Arc<RenderState>) {
    let options = Rc::new(render_state.options.clone());
    let mut thread_state = ThreadState::new(thread_id, options.clone());

    let height = options.height as f64;
    let width = options.width as f64;
    let scale = options.scale;
    let p = &options.center_point;

    let time = Utc::now();

    let y = 2. * p.lat_sin;
    let k = (0.5 * y * width * scale / PI + 0.5) as i32;

    for h in 0..thread_state.local_height as i32 {
        let real_h = h + thread_state.starting_line as i32;
        let y = 0.5 * PI * (2 * (real_h - k) - height as i32) as f64 / (width * scale);

        if y.abs() > 1. {
            for w in 0..options.width as usize {
                thread_state.canvas[h as usize][w] = render_state.color_table.back;
                if options.shading_level > 0 {
                    thread_state.shading[h as usize][w] = 255;
                }
            }
        } else {
            let cos2 = (1. - y * y).sqrt();
            if cos2 > 0. {
                let scale1 = scale * width / (height * cos2 * PI);
                thread_state.starting_subdivision_depth = (scale1 * height).log2() as u8 * 3 + 3;

                for w in 0..options.width as usize {
                    let theta =
                        p.longitude - 0.5 * PI + PI * (2. * w as f64 - width) / (width * scale);
                    let world_point =
                        Vertex::from_point(theta.cos() * cos2, y, -theta.sin() * cos2);
                    render_pixel(&mut thread_state, &world_point, w, h as usize);
                }
            }
        }
        if h > 0 && h % 100 == 0 {
            let millis = (Utc::now() - time).num_milliseconds();
            let pixels_per_second = (h as i64 * options.width as i64) / millis * 1000;
            println!("Thread {thread_id} completed line {h} - {pixels_per_second}pps",);
        }
    }
    commit_render_data(thread_id, thread_state, render_state.clone());
}
